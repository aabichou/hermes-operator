...
test: manifests generate fmt vet envtest ## Run tests.
	KUBEBUILDER_ASSETS="$(shell KUBEBUILDER_ASSETS=$(LOCALBIN)/k8s/$(ENVTEST_K8S_VERSION)-$(shell go env GOOS)-$(shell go env GOARCH) $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) --use-env -p path 2>/dev/null || $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" go test $$(go list ./... | grep -v /e2e) -coverprofile cover.out
